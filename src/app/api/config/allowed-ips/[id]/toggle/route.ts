...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const res = await fetch(`${API_ENDPOINTS.BASE_URL}${API_ENDPOINTS.ENDPOINTS.CONFIG.ALLOWED_IPS_TOGGLE(params.id)}`, {
      method: "PUT",
      headers: { 
    ...
    if (!res.ok) {
      return NextResponse.json({ 
        error: camelData.error || camelData.message || "Erro ao alterar status do IP" 
      }, { status: res.status });
    }
    ...
    return NextResponse.json({ 
      error: "Erro interno do servidor ao alterar status do IP" 
    }, { status: 500 });
  }
}