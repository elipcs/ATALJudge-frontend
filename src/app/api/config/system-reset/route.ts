...
export async function POST(request: NextRequest) {
  try {
    ...
    const res = await fetch(`${API_ENDPOINTS.BASE_URL}${API_ENDPOINTS.ENDPOINTS.CONFIG.SYSTEM_RESET}`, {
      method: "POST",
      headers: { 
    ...
    if (!res.ok) {
      return NextResponse.json({ 
        error: camelData.error || camelData.message || "Erro ao realizar reset do sistema" 
      }, { status: res.status });
    }
    return NextResponse.json(camelData);
    
  } catch (error) {
    console.error("Erro ao realizar reset do sistema:", error);
    return NextResponse.json({ 
      error: "Erro interno do servidor ao realizar reset do sistema" 
    }, { status: 500 });
  }
}