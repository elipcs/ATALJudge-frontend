  ...
  try {
    ...
    if (!res.ok) {
      return NextResponse.json({ 
        error: data.message || data.error || "Erro ao buscar submissões" 
      }, { status: res.status });
    }
...
export async function POST(req: Request) {
  try {
    ...
    const res = await fetch(`${API_ENDPOINTS.BASE_URL}/api/submissions`, {
      method: "POST",
      headers: { 
        "Content-Type": "application/json",
        ...(req.headers.get('authorization') ? { 'Authorization': req.headers.get('authorization')! } : {})
    ...
    if (!res.ok) {
      return NextResponse.json({ 
        error: data.message || data.error || "Erro ao criar submissão" 
      }, { status: res.status });
    }
