...
export default function ProfileIdPage() {
  ...
  const id = Array.isArray(params?.id) ? params.id[0] : params?.id;
	const userData = mockUsers[String(id)];
	// Se não existir, não renderiza nada (ou poderia redirecionar/mostrar erro)
	if (!userData) {
		return (
			<div className="min-h-screen flex items-center justify-center">
				<div className="bg-white rounded-xl shadow p-8 text-center">Usuário não encontrado</div>
			</div>
		);
	}
	...
	// Debug: log do valor de role
	if (typeof window !== "undefined") {
		// eslint-disable-next-line no-console
		console.log("[ProfileIdPage] role:", role);
	}
  ...
  const [matriculaState, setMatricula] = useState(matricula || "");
	...
	const user = { id: String(id || ""), avatar };
	return (
